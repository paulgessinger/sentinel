...
GITHUB_APP_ID: int = int(os.environ["GITHUB_APP_ID"])

# Global log level override for the service.
OVERRIDE_LOGGING: int = getattr(
...
_repo_allowlist_raw: str | None = os.environ.get("REPO_ALLOWLIST")
REPO_ALLOWLIST: list[str] | None = (
    _repo_allowlist_raw.split(",") if _repo_allowlist_raw is not None else None
)
...
OVERRIDE_CONFIG: str | None = os.environ.get("OVERRIDE_CONFIG")

# Startup pause for worker/process boot sequencing.
PROCESS_START_PAUSE: float = float(os.environ.get("PROCESS_START_PAUSE", 5))

# Regex to skip specific check_run names/status contexts early.
CHECK_RUN_NAME_FILTER: str | None = os.environ.get("CHECK_RUN_NAME_FILTER")

# Diskcache root (required) used by queue/cache and default SQLite path.
DISKCACHE_DIR: str = os.environ["DISKCACHE_DIR"]

# Cooldown window per PR before reprocessing from queue.
PR_TIMEOUT: float = float(os.environ.get("PR_TIMEOUT", 10))

# Worker polling interval for queue loop.
WORKER_SLEEP: float = float(os.environ.get("WORKER_SLEEP", 1))

# Installation token cache TTL in seconds.
ACCESS_TOKEN_TTL: float = float(os.environ.get("ACCESS_TOKEN_TTL", 300))

# TTL for cached repo PR lists used by legacy webhook dispatch path.
PRS_TTL: float = float(os.environ.get("PRS_TTL", 60))

# Legacy worker dry-run: compute only, skip GitHub writes.
DRY_RUN: bool = os.environ.get("DRY_RUN", "false") == "true"

# Legacy debounce window for duplicate check/status webhook suppression.
CHECK_RUN_DEBOUNCE_WINDOW: float = float(
    os.environ.get("CHECK_RUN_DEBOUNCE_WINDOW", 60 * 60)
)

# Enable/disable webhook SQLite persistence and projections.
WEBHOOK_DB_ENABLED: bool = (
    os.environ.get("WEBHOOK_DB_ENABLED", "true").lower() == "true"
)
# SQLite file path for webhook event log + projection tables.
WEBHOOK_DB_PATH: str = os.environ.get(
...
_webhook_retention_seconds_raw: str | None = os.environ.get(
    "WEBHOOK_DB_RETENTION_SECONDS"
)
if _webhook_retention_seconds_raw is not None:
    WEBHOOK_DB_RETENTION_SECONDS: int = int(_webhook_retention_seconds_raw)
else:
...
WEBHOOK_PROJECTION_COMPLETED_RETENTION_SECONDS: int = int(
...
WEBHOOK_PROJECTION_ACTIVE_RETENTION_SECONDS: int = int(
...
WEBHOOK_FILTER_APP_IDS: tuple[int, ...] = tuple(
    int(part.strip())
...
PROJECTION_DEBOUNCE_SECONDS: float = float(
...
PROJECTION_PULL_REQUEST_SYNCHRONIZE_DELAY_SECONDS: float = float(
    os.environ.get("PROJECTION_PULL_REQUEST_SYNCHRONIZE_DELAY_SECONDS", 15)
)
# Name used for Sentinel's own check run.
PROJECTION_CHECK_RUN_NAME: str = os.environ.get(
